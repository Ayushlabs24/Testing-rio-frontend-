...
import { ncnpReportService } from "@/services/ncnp-report/ncnp-report.service";
import type { NcnpReport } from "@/services/ncnp-report/ncnp-report.types";
import { KpiDeltaCard } from "./_components/kpi-delta-card";
import { NamedBarList } from "./_components/named-bar-list";
import { PieChart } from "./_components/pie-chart";
import { RegionMap } from "./_components/region-map";
import { ReportPageShell } from "./_components/report-page-shell";
import { Sparkline } from "./_components/sparkline";
import { StatusDonut } from "./_components/status-donut";
import { TrendLineChart } from "./_components/trend-line-chart";
import { TwoStateBar } from "./_components/two-state-bar";
...
function NcnpReportContent() {
  const t = useTranslations("systemAdmin.ncnpReport");
  const { session } = useAuth();
  const router = useRouter();
  const [report, setReport] = useState<NcnpReport | null>(null);
  ...
  async function handleExport(format: "pdf" | "excel") {
    ...
    try {
      await ncnpReportService.download(format);
    } catch {
      setExportError(true);
    } finally {
  ...
  // PermissionGuard below only checks archiveSharingAudit:read — several
  // non-cross-entity roles hold that too (it also gates Archive/Sharing).
  // The backend hard-403s any non-crossEntity role for this specific
  // endpoint, so this page enforces the same restriction itself rather
  // than relying on the nav link being hidden (direct URL navigation must
  // be blocked too) — same redirect-away pattern PermissionGuard itself uses.
  useEffect(() => {
    if (session && !session.role.crossEntity) {
      router.replace("/dashboard");
    }
  }, [session, router]);

  useEffect(() => {
    if (!session?.role.crossEntity) return;
    ncnpReportService
      .get()
      .then((data) => {
        setReport(data);
  ...
  if (!session?.role.crossEntity) return null;
  ...
  return (
    <PageContainer>
          ...
          <div className="border-border mb-6 border-b-2 pb-5">
            ...
            <div className="border-border/60 bg-muted/30 mt-5 flex flex-wrap gap-x-8 gap-y-2 rounded-xl border p-4 text-sm">
              ...
              <div>
                ...
                <span className="text-foreground font-semibold">{session.user.name}</span>
              </div>
            </div>
          </div>
        }
      ...
      <div className="text-muted-foreground border-border mt-4 flex items-center justify-between border-t-2 pt-4 text-xs">
        ...
        <span>
          {t("generatedBy")}: {session.user.name} · {reportId(report.generatedAt)}
        </span>
      </div>
    </PageContainer>
  );
...
export default function NcnpReportPage() {
  return (
